<template>
  <view class="page">
    <view class="topbar">
      <text class="topbar-action" @tap="goPrevious">返回</text>
      <view class="topbar-center">
        <text class="phase-title">{{ showComplete ? '问卷生成' : phaseLabel }}</text>
        <text v-if="!showComplete" class="progress-text">第 {{ screenIndex + 1 }} 步 / 共 {{ screenTotal }} 步</text>
      </view>
      <text class="topbar-action topbar-action--right" @tap="restart">重置</text>
    </view>
    ...
    <scroll-view v-else class="content" scroll-y :scroll-top="scrollTop" :scroll-into-view="scrollIntoView" scroll-with-animation :enable-back-to-top="true">
      ...
      <view v-if="isCurrentBatchScreen" class="question-card">
        <view class="question-head">
          <text class="question-badge">{{ phaseLabel }}</text>
          <text class="question-index">第 {{ screenIndex + 1 }} 步</text>
        </view>

        <ChatAssetImage
          v-if="batchStageArtworkPath"
          :path="batchStageArtworkPath"
          mode="aspectFit"
        ...
        <text class="batch-section-title">{{ batchScreenTitle }}</text>
        <text v-if="batchScreenDesc" class="batch-section-desc">{{ batchScreenDesc }}</text>
        ...
        >
          <template v-if="isPersonalityBatch">
            <view class="batch-block-head">
              <text class="batch-block-title">{{ bi + 1 }}. {{ bq.prompt }}</text>
              <text class="batch-block-meta">{{ bi + 1 }}/{{ screenQuestions.length }}</text>
            </view>
          </template>
      ...
      <view v-else-if="q" class="question-card">
        ...
        <view class="question-body">
          ...
          <view id="anchor-career-items" class="career-section">
            <ChatAssetImage
              ...
              label="情景配图"
            />
            <text class="career-scene-text">{{ q.careerScenario.itemScene || q.careerScenario.scene }}</text>
            ...
            <view v-if="careerVal.itemChoiceIds.length" class="career-rank-selected">
              <view
                v-for="(optionId, ri) in careerVal.itemChoiceIds"
                :key="'rank-' + optionId"
                class="career-rank-slot career-rank-slot--on"
                :class="{
                  'career-rank-slot--drag': rankDragIndex === ri,
                  'career-rank-slot--drop': rankDropTargetIndex === ri && rankDragIndex >= 0 && rankDragIndex !== ri,
                }"
                :style="rankSlotDragStyle(ri)"
                @touchstart="onCareerRankTouchStart(ri, $event)"
                @touchmove.stop.prevent="onCareerRankTouchMove"
            ...
            <text v-if="careerVal.itemChoiceIds.length" class="career-rank-hint">拖动可调整优先级顺序，向下滑动移回下方；轻点取消选择</text>
            <view v-if="careerVal.itemChoiceIds.length" class="career-rank-divider"></view>

            <view
              class="career-item-pool"
              :class="{ 'career-item-pool--err': careerFieldErrors.itemChoiceIds }"
            >
              <view
          ...
          <view id="anchor-career-team" class="career-section">
            <ChatAssetImage
              ...
              label="情景配图"
            />
            <text class="career-scene-text">{{ q.careerScenario.openScene || q.careerScenario.openPrompt }}</text>
            <text class="career-block-title">{{ careerTeamTitle }}</text>
    ...
    <view class="footer">
      ...
      <template v-else>
        <button class="fbtn fbtn--ghost" @tap="goPrevious">上一题</button>
        <button class="fbtn fbtn--primary" :disabled="finishing" @tap="handlePrimary">{{ primaryLabel }}</button>
      </template>
    </view>
...
import { ALL_QUESTIONS, createEmptyCareerScenarioAnswer, isCareerScenarioValue } from '../../business/discover-questions'
import {
  CHAT_STAGES,
...
import { clearProfileUploadFlag } from '../../business/profile-sync'
...
const batchScreenTitle = computed(() => getScreenTitle(currentScreenId.value, screenQuestions.value))
const batchScreenDesc = computed(() => {
  if (currentScreenId.value !== 'ability') return ''
  return getAbilityBatchDescription(screenQuestions.value)
})
const batchStageArtworkPath = computed(() => {
  const stageIdx = getScreenStageIndex(currentScreenId.value)
  return CHAT_STAGES[stageIdx]?.artwork || ''
})

const phaseLabel = computed(() => {
  const idx = showComplete.value ? 3 : getScreenStageIndex(currentScreenId.value)
  return CHAT_STAGES[idx]?.label || CHAT_STAGES[0].label
})
const activeStageIndex = computed(() => (showComplete.value ? 3 : getScreenStageIndex(currentScreenId.value)))
const stageFillPct = computed(() => {
  const idx = activeStageIndex.value
  return chatStages.length <= 1 ? 100 : (idx / (chatStages.length - 1)) * 100
})
...
const stageArtworkPath = computed(() => {
  if (showComplete.value) return CHAT_STAGES[3].artwork
...
const rankDragOffsetX = ref(0)
const rankDragOffsetY = ref(0)
...
function checkReady() {
  if (isCurrentBatchScreen.value) {
    ...
    primaryLabel.value = screenIndex.value >= screenTotal - 1 ? '完成' : '下一题'
    return
  }
...
function restart() {
  ...
  uni.removeStorageSync('uniprism.discoverReport')
  showComplete.value = false
...
function rankSlotDragStyle(index) {
  if (rankDragIndex.value === index) {
    return `transform: translate(${rankDragOffsetX.value}px, ${rankDragOffsetY.value}px); opacity: 0.92; z-index: 10;`
  }
  return ''
}

function measureRankSlots() {
  return new Promise((resolve) => {
    const query = uni.createSelectorQuery()
    if (pageInstance && pageInstance.proxy) query.in(pageInstance.proxy)
    query
      .selectAll('.career-rank-selected .career-rank-slot')
      .boundingClientRect((rects) => {
        rankSlotRects.value = Array.isArray(rects) ? rects : []
...
function resetRankDrag() {
  ...
  rankDragOffsetX.value = 0
  rankDragOffsetY.value = 0
  rankDragMoved.value = false
  rankDropTargetIndex.value = -1
}

function onCareerRankTouchStart(index, e) {
  if (!careerVal.value.itemChoiceIds[index]) return
...
function onCareerRankTouchMove(e) {
  if (rankDragIndex.value < 0) return
  const touch = e.touches && e.touches[0]
  if (!touch) return
  const dx = touch.clientX - rankDragStartX.value
  const dy = touch.clientY - rankDragStartY.value
  if (Math.abs(dx) > 8 || Math.abs(dy) > 8) rankDragMoved.value = true
  rankDragOffsetX.value = dx
  rankDragOffsetY.value = dy
  rankDropTargetIndex.value = getRankSlotIndexFromPoint(touch.clientX, touch.clientY)
}

function applyCareerRankReorder(sourceIndex, targetIndex) {
  ...
  const boundedTarget = Math.max(0, Math.min(max - 1, targetIndex))
  if (boundedTarget === sourceIndex) return

  let next
  if (boundedTarget < selected.length) {
    next = selected.slice()
...
function onCareerRankTouchEnd(e) {
  ...
  const dx = touch ? touch.clientX - rankDragStartX.value : rankDragOffsetX.value
  const dy = touch ? touch.clientY - rankDragStartY.value : rankDragOffsetY.value

  if (rankDragMoved.value) {
    if (dy > 48 && dy > Math.abs(dx)) {
      clearCareerRankSlot(index)
    } else {
      const target = rankDropTargetIndex.value >= 0
        ? rankDropTargetIndex.value
        : (touch ? getRankSlotIndexFromPoint(touch.clientX, touch.clientY) : -1)
      if (target >= 0 && target !== index) applyCareerRankReorder(index, target)
    }
  } else {
    clearCareerRankSlot(index)
  }
...
.puzzle-image-wrap { min-height: 240rpx; }
...
.profile-hero-wrap { width: 192rpx; height: 192rpx; min-height: 192rpx; border-radius: 24rpx; }
...
.tag-tile { width: calc(50% - 7rpx); box-sizing: border-box; display: flex; flex-direction: column; align-items: center; padding: 24rpx 20rpx 20rpx; border-radius: 8rpx; border: 3rpx solid #e5e5e5; background: #fff; box-shadow: 0 4rpx 0 #e5e5e5; }
.tag-tile--on { border-color: rgba(107,35,255,0.1); background: rgba(107,35,255,0.1); box-shadow: 0 4rpx 0 #9762ff; }
.tag-icon-wrap { width: 116rpx; height: 116rpx; border-radius: 40rpx; background: #fff; display: flex; align-items: center; justify-content: center; overflow: hidden; }
.tag-icon-inner { width: 104rpx; height: 104rpx; min-height: 104rpx; }
.tag-icon { width: 104rpx; height: 104rpx; display: block; }
...
.career-item-pool { display: flex; flex-wrap: wrap; gap: 16rpx; margin-top: 20rpx; }
.career-item-tile {
  width: calc(50% - 8rpx);
  min-height: 88rpx;
  box-sizing: border-box;
...
.career-opt--on { border-color: rgba(107,35,255,0.1); background: rgba(107,35,255,0.1); box-shadow: 0 4rpx 0 #9762ff; }
.career-opt--on .opt-p { background: #fff; color: #6b23ff; }
.career-opt--on .opt-l { color: #4b4b4b; }
.career-rank-selected { display: flex; flex-direction: row; flex-wrap: wrap; gap: 12rpx; margin-top: 20rpx; }
.career-rank-selected .career-rank-slot { flex: 1 1 calc(50% - 6rpx); width: auto; min-width: 0; min-height: 88rpx; box-sizing: border-box; display: flex; align-items: center; justify-content: center; padding: 16rpx 14rpx; border-radius: 8rpx; border: 3rpx solid rgba(107,35,255,0.1); background: rgba(107,35,255,0.1); box-shadow: 0 4rpx 0 #9762ff; }
.career-rank-slot { position: relative; }
.career-rank-slot--on { border-color: rgba(107,35,255,0.1); background: rgba(107,35,255,0.1); box-shadow: 0 4rpx 0 #9762ff; }
.career-rank-slot--drag { z-index: 10; }
.career-rank-slot--drop { border-color: #6b23ff; background: #f3edff; box-shadow: 0 0 0 2rpx rgba(107,35,255,0.2); }
.career-rank-slot-text { font-size: 24rpx; line-height: 1.35; color: #4b4b4b; font-weight: 700; text-align: center; width: 100%; word-break: break-word; }
...
.batch-opt.opt--on .opt-l { color: #4b4b4b; font-weight: 500; }
...
.complete-desc { display: block; margin-top: 28rpx; font-size: 28rpx; line-height: 1.8; color: #4b4b4b; }
.complete-hint { display: block; margin-top: 20rpx; font-size: 24rpx; line-height: 1.7; color: #7a8292; }
