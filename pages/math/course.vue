...
<script>
import { getMathCourse } from '../../business/math-catalog'
import { resolveAsset } from '../../utils/asset-map'

export default {
  data() {
    return { courseId: 'analysis', pageIndex: 0 }
  },
  onLoad(options) {
    if (options && options.course) this.courseId = options.course
  },
  computed: {
    course() { return getMathCourse(this.courseId) },
    topic() {
  ...
  methods: {
    resolveAsset,
    goPrev() {
      if (this.pageIndex <= 0) { uni.navigateTo({ url: '/pages/math/index' }); return }
      this.pageIndex--
    },
    goNext() {
      if (this.pageIndex >= this.course.pages.length - 1) { uni.navigateTo({ url: '/pages/math/index' }); return }
      this.pageIndex++
    }
  }
}
