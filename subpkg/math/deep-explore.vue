  ...
  <view class="page">
    ...
    <view class="roadmap">
      ...
      <view class="map-arrow arrow-foundation" :class="isRoadNodeActive('foundation') ? 'arrow--teal' : 'arrow--gray'" @tap="openStage('foundation')">
        ...
        <text class="map-arrow-text">数学课程介绍</text>
      </view>
...
export default {
  ...
  methods: {
    ...
    handleContinue() {
      ...
      uni.redirectTo({ url: `/subpkg/math/index?pageIndex=${pageIndex}` })
    },
    navigateToProgress() {
      uni.navigateTo({ url: '/subpkg/math/progress' })
    },
...
.connector-branch {
  left: 154rpx;
  top: calc(915rpx + var(--finish-shift-y));
}
...
.arrow-finish {
  right: 58rpx;
  top: calc(942rpx + var(--finish-shift-y));
}
...
.icon-finish {
  right: 96rpx;
  top: calc(958rpx + var(--finish-shift-y));
  width: 132rpx;
  height: 132rpx;
  overflow: visible;
}
