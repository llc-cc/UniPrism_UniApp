<template>
  ...
  >
    ...
    <scroll-view
      ...
      :scroll-y="!isPersonalityStageIntro"
    ...
    >
      ...
      >
        ...
        <view class="stage-intro-hero">
          ...
          />
          <view
            v-if="(isPersonalityStageIntro || isHollandStageIntro) && currentStageIntro.iconSrc"
            :class="currentStageIntro.iconWrapClass || 'stage-intro-icon-wrap'"
          >
            <image
              :src="resolveAsset(currentStageIntro.iconSrc)"
              mode="aspectFit"
              :class="currentStageIntro.iconClass || 'stage-intro-icon'"
            />
          </view>
          <ChatAssetImage
            v-else-if="currentStageIntro.iconSrc"
      ...
      >
        ...
        <view v-if="q.type === 'holland-fine-grained' && q.hollandFine" class="holland-fine">
          <text class="batch-section-title holland-fine-section-title">{{ hollandFineDisplayTitle }}</text>
          <view class="holland-fine-scene holland-fine-scene--top">
            <ChatAssetImage
              v-if="hollandFineSceneImage"
              :path="hollandFineSceneImage"
              mode="aspectFit"
              ...
              :show-path-hint="0"
            />
            <text v-if="hollandFineSceneText" class="batch-block-prompt holland-fine-scene-text">{{ hollandFineSceneText }}</text>
          </view>
          <view
          ...
          >
            <view class="holland-fine-question">
              <view class="career-block-head holland-fine-question-header">
                <text class="batch-block-prompt holland-fine-question-title">{{ '问题' + (itemIndex + 1) + '｜' + item.prompt }}</text>
                <text v-if="getHollandFineItemMeta(item)" class="batch-block-meta">{{ getHollandFineItemMeta(item) }}</text>
              </view>
              ...
              <view
                v-if="item.mode === 'scale-pair'"
                class="holland-fine-scale-group"
              >
                <view
                  v-for="(opt, oi) in item.options || []"
                  :key="opt.id"
                  class="holland-fine-scale-choice"
                  :class="{
                    'holland-fine-scale-choice--on': hollandFineOptionSelected(item.id, opt.id),
                    'holland-fine-scale-choice--err': hollandFineFieldErrors[item.id] && !hollandFineOptionSelected(item.id, opt.id),
                  }"
                  @tap="setHollandFineOption(item, opt.id)"
                >
                  <view
                    class="holland-fine-scale-circle"
                    :class="{
                      'holland-fine-scale-circle--on': hollandFineOptionSelected(item.id, opt.id),
                      'holland-fine-scale-circle--err': hollandFineFieldErrors[item.id] && !hollandFineOptionSelected(item.id, opt.id),
                    }"
                  >
                    <text class="holland-fine-scale-circle-text">{{ chr(oi) }}</text>
                  </view>
                  <text class="holland-fine-scale-choice-label">{{ opt.label }}</text>
                </view>
              </view>
              <text
                v-if="item.mode === 'scale-pair' && hollandFineFieldErrors[item.id]"
                class="validation-inline-text"
              >
                {{ hollandFineFieldErrors[item.id] }}
              </text>
              <view
                v-if="item.mode === 'open'"
              ...
              <view
                v-else-if="item.mode !== 'scale-pair'"
                class="batch-opt-grid batch-opt-grid--stack holland-fine-choice-list"
              >
                <view
                  v-for="(opt, oi) in item.options || []"
                  :key="opt.id"
                  ...
                  :class="{
                    'career-opt--on': hollandFineOptionSelected(item.id, opt.id),
              ...
              <text v-if="item.mode !== 'scale-pair' && hollandFineFieldErrors[item.id]" class="validation-inline-text">
                {{ hollandFineFieldErrors[item.id] }}
              </text>
              <text
                v-if="item.mode !== 'scale-pair' && (item.mode === 'multi' || item.mode === 'ranked' || item.mode === 'primary-secondary') && getHollandFineMaxSelections(item) > 1"
...
import {
  ...
  ensureBasicProfileBeforeReport,
  hasBasicProfileAnswer,
...
const profileOnlyMode = ref(false)
const navLayout = ref({
  paddingTop: 44,
  contentHeight: 44,
  sideWidth: 96,
...
const stageIntroConfigs = {
  'stage-personality-intro': {
    ...
    iconSrc: '/static/assets/auth/card-personality.svg',
    shadowSrc: '',
    ...
    primaryLabel: '点击测试',
    metrics: ['预计时长：大约5分钟', `题目数量：${PERSONALITY_MBTI_DISPLAY_QUESTION_IDS.length}题`],
    paragraphs: [],
  },
  'stage-holland-intro': {
    badge: '',
    ...
    iconSrc: '/static/assets/auth/card-career.svg',
    shadowSrc: '',
    ...
    primaryLabel: '点击测试',
    metrics: ['预计时长：大约5分钟', `题目数量：${PERSONALITY_HOLLAND_SCALE_DISPLAY_QUESTION_IDS.length}题`],
    paragraphs: [],
  },
  'stage-deep-intro': {
    badge: '',
    ...
    iconSrc: '/static/assets/discover/stage-deep-icon-hd.webp',
    shadowSrc: '',
    ...
    primaryLabel: '点击测试',
    metrics: ['预计时长：大约8分钟', `题目数量：约${10 + FINAL_CAREER_CALIBRATION_QUESTION_IDS.length}道题`],
    paragraphs: [],
  },
}
...
const hollandFineSceneImage = computed(() => {
  const current = q.value
  if (!current || current.type !== 'holland-fine-grained' || !current.hollandFine) return ''
  const firstItem = current.hollandFine.items?.[0]
  return (firstItem && firstItem.sceneImageSrc) || current.imageSrc || ''
})

const hollandFineSceneText = computed(() => {
  const current = q.value
  if (!current || current.type !== 'holland-fine-grained' || !current.hollandFine) return ''
  const firstItem = current.hollandFine.items?.[0]
  return (firstItem && firstItem.scene) || ''
})
...
function goToNextScreen() {
  ...
  if (profileOnlyMode.value && isBasicProfileScreen.value) {
    void continueAfterProfileOnly(savedSession)
    return
  }
...
function bootstrapPage(options = {}) {
  ...
  if (!options.skipResultsRedirect && session.phase === 'results' && session.profile) {
    uni.redirectTo({ url: '/subpkg/discover/results' })
    return
  }
...
function forceStartFromPersonalityIntro() {
  const session = loadDiscoverSession()
  const nextSession = {
    ...session,
    phase: 'chatting',
    currentQuestionIndex: getScreenIndexById('stage-personality-intro'),
    backNavigationFloorIndex: null,
  }
  saveDiscoverSession(nextSession)
}

function maybeSkipBasicProfileForAnonymousExplore() {
  const session = loadDiscoverSession()
  if (hasBasicProfileAnswer(session)) return
  const basicIndex = getScreenIndexById('basic')
  if (screenIndex.value !== basicIndex) return
  const answers = session.answers || []
  const hasAssessmentAnswers = answers.some((item) => item.questionId !== 'basic-profile')
  if (hasAssessmentAnswers) {
    const resumeIndex = Math.max(
      boundedScreenIndex(session.currentQuestionIndex, screenTotal),
      findNextRenderableScreenIndex(basicIndex + 1, answers) ?? getScreenIndexById('stage-personality-intro'),
    )
    jumpToScreenIndex(resumeIndex, {
      forceBackNavigationFloorIndex: getScreenIndexById('stage-personality-intro'),
    })
    return
  }
  jumpToScreenIndex(getScreenIndexById('stage-personality-intro'), {
    forceBackNavigationFloorIndex: getScreenIndexById('stage-personality-intro'),
  })
}
...
async function prepareChatEntry(options) {
  ...
  profileOnlyMode.value = normalizeBooleanOption(options && options.profile)
  const shouldRetake = normalizeBooleanOption(options && options.retake)
  const shouldStartFresh = normalizeBooleanOption(options && options.start)
  ...
  try {
    ...
    if (profileOnlyMode.value) {
      await ensureBackendSessionReady()
      bootstrapPage({ skipResultsRedirect: true })
      jumpToScreenIndex(getScreenIndexById('basic'))
    } else if (shouldRetake) {
      await restart({ silent: true })
      bootstrapPage({ skipResultsRedirect: true })
    } else {
      await ensureBackendSessionReady()
      if (shouldStartFresh) {
        forceStartFromPersonalityIntro()
      }
      bootstrapPage({ skipResultsRedirect })
      maybeSkipBasicProfileForAnonymousExplore()
    }

    if (!profileOnlyMode.value && targetScreen === 'complete') {
      jumpToScreenIndex(-1)
    } else if (!profileOnlyMode.value && targetScreen) {
...
async function generateReport() {
  if (!(await promptLoginForReport('/subpkg/discover/chat?screen=complete'))) return
  const session = loadDiscoverSession()
  if (!ensureBasicProfileBeforeReport(session)) return
  await finish(session)
}

async function continueAfterProfileOnly(session) {
  if (!(await promptLoginForReport('/subpkg/discover/chat?profile=1'))) return
...
async function finish(session) {
  if (!(await promptLoginForReport('/subpkg/discover/chat?screen=complete'))) return
  if (!ensureBasicProfileBeforeReport(session || loadDiscoverSession())) return
  clearExploreBackendSession()
  clearProfileUploadFlag()
  clearDiscoverReportCache()
  ...
  if (user && user.id && hasBasicProfileAnswer(done)) {
    markBasicProfileCompleted(user.id)
  }
  clearPendingReportGeneration()
  finishing.value = true
  try {
    uni.redirectTo({ url: '/subpkg/discover/results?generate=1' })
  } finally {
    finishing.value = false
  }
}
...
.stage-intro-card--deep .stage-intro-icon--deep-launch {
  position: relative;
  z-index: 2;
  filter: drop-shadow(0 9rpx 0 rgba(61, 10, 168, 0.98))
    drop-shadow(0 16rpx 7rpx rgba(61, 10, 168, 0.18));
...
:deep(.calibration-question-image-wrap) {
  width: 100%;
  margin: 48rpx auto 24rpx;
  min-height: 345rpx;
  display: flex;
...
:deep(.calibration-question-image) {
  width: 460rpx;
  height: 345rpx;
  display: block;
  flex-shrink: 0;
...
.batch-opt--scale.opt--on,
.batch-opt--scale.opt--on.opt--err {
  background: #fff !important;
  border-color: #e6e6e6 !important;
  box-shadow: 0 4rpx 0 #e6e6e6 !important;
}
...
.batch-opt:not(.batch-opt--scale).opt--on {
  background: #f0e8ff !important;
  border-color: #9761ff !important;
  box-shadow: 0 4rpx 0 2rpx #9761ff !important;
}
...
.holland-fine-scene--top {
  margin-top: 0;
}
:deep(.holland-fine-scene-image-wrap) {
  width: 100%;
  margin: 0 auto;
  min-height: 345rpx;
  display: flex;
...
.holland-fine-scale-circle-text {
  font-size: 28rpx;
  line-height: 1;
  font-weight: 800;
  color: #999999;
}
.holland-fine-scale-choice-label {
  margin-top: 12rpx;
  font-size: 22rpx;
  line-height: 1.35;
  font-weight: 700;
...
.holland-fine-scale-circle--on {
  border-color: #6b23ff;
  background: #6b23ff;
  box-shadow: 0 8rpx 0 #3e0aa9;
}
.holland-fine-scale-circle--on .holland-fine-scale-circle-text {
  color: #ffffff;
}
.holland-fine-scale-circle--err {
  border-color: #ff4d4f;
}
...
/* Keep the stage intro heading size consistent across stages 1-3. */
.stage-intro-card--personality .stage-intro-title,
.stage-intro-card--holland .stage-intro-title,
.stage-intro-card--deep .stage-intro-title {
  font-size: 50rpx;
...
/* 阶段一、二、三共享同一套几何尺寸，单独保留各自的底托表现 */
.stage-intro-card--personality .stage-intro-hero::before,
.stage-intro-card--holland .stage-intro-hero::before,
.stage-intro-card--deep .stage-intro-hero::before {
  width: 359rpx !important;
  height: 209rpx !important;
  top: 58% !important;
  border-radius: 40% 60% 48% 52% / 62% 30% 70% 38% !important;
...
.stage-intro-card--personality .stage-intro-icon--personality-launch {
  transform: translateY(4rpx) scale(1.2) !important;
}
.stage-intro-card--deep .stage-intro-icon--deep-launch {
  filter: drop-shadow(0 9rpx 0 rgba(61, 10, 168, 0.98))
    drop-shadow(0 16rpx 7rpx rgba(61, 10, 168, 0.18)) !important;
}
.stage-intro-card--personality {
  padding-top: 210rpx !important;
}
.stage-intro-card--personality .stage-intro-hero {
  min-height: 296rpx !important;
