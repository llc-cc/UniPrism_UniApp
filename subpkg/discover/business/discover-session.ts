...
import {
  ...
  type RIASECProfile,
  type RIASECScores,
} from '../../../business/riasecEngine';
import { readStorage, removeStorage, writeStorage } from '../../../business/storage';
...
/** 登录前已完成测评但未 finish 时，先补齐本地 profile 再生成报告。 */
export function prepareDiscoverSessionForReport(): DiscoverSession {
  const session = loadDiscoverSession();
  if (session.profile) return session;
  if (!Array.isArray(session.answers) || session.answers.length === 0) return session;
